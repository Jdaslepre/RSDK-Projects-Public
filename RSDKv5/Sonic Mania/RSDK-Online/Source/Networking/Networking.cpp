...
char networkHost[64];
char networkGame[7] = "RSDK";
int32 networkPort   = 50;
int32 dcError       = 0;
float lastPing      = 0;

bool32 waitingForPing = false;
...
public:
    ...
    void start() {
        ...
        write_msgs_.push_back(sent);
        Dev::Print(Dev::PRINT_NORMAL, "Networking::Start -> Starting session...");
    }
    ...
    void close() {
        ...
        Dev::Print(Dev::PRINT_NORMAL, "Networking::Close -> Stopping session...");
    }
    ...
    bool32 awaitingReceive = false;
    bool32 verifyReceived  = false;
    ...
    void run() {
        ...
        while (!write_msgs_.empty()) {
            ...
            Misc::StrCopy(send->game, networkGame);
            socket.send_to(asio::buffer(send, sizeof(ServerPacket)), endpoint);
            write_msgs_.pop_front();
            if (send->header == 0xFF) {
                session->running = false;
                Dev::Print(Dev::PRINT_NORMAL, "Networking::Run -> Stopping session... (send->header == 0xFF)");
            }
        }
        if (!awaitingReceive)
            do_read();
        if (repeat.header != 0x80 && retried) {
            ...
            timer.async_wait([&](const asio::error_code &) {
                retried     = true;
                repeat.room = room;
                Misc::StrCopy(repeat.game, networkGame);
                if (retries++ == 10)
        ...
        if (retries > 10) {
            switch (repeat.header) {
                case 0x01: {
                    ...
                    vsPlaying        = false;
                    session->running = false;
                    Dev::Print(Dev::PRINT_NORMAL, "Networking::Run -> Couldn't get room code after 10 retries, stopping session.");
                    break;
                }
                case 0x00: {
                    if (!room) {
    ...
    void leave() {
        ...
        vsPlaying   = false;
        write(send);
        Dev::Print(Dev::PRINT_NORMAL, "Networking::Leave -> Leaving networking session");
    }
    ...
    void do_read() {
        ...
        socket.async_receive(asio::buffer(&read_msg_, sizeof(ServerPacket)), [&](const asio::error_code &ec, size_t bytes) {
            ...
            switch (read_msg_.header) {
                ...
                // fallthrough
                case SV_DATA: {
                    UserData::Receive2PVSData(&read_msg_.data.multiData);
                    return;
                }
                case SV_RECEIVED: {
...
void InitNetwork() {
    try {
        ...
        Dev::Print(Dev::PRINT_NORMAL, "Networking initialized");
    } catch (std::exception &e) {
        onlineActive = false;
        Dev::Print(Dev::PRINT_NORMAL, "Failed to initialize networking: %s", e.what());
    }
}

void networkLoop() {
    try {
        Dev::Print(Dev::PRINT_NORMAL, "Networking::Loop -> session->start()");
        session->start();
        while (session->running) session->run();

        Dev::Print(Dev::PRINT_NORMAL, "Networking::Loop -> session->close()");
        session->close();
    } catch (std::exception &e) {
...
void RunNetwork() {
    if (loopThread.joinable()) {
        Dev::Print(Dev::PRINT_NORMAL, "Networking::Run -> loopThread.joinable()");
        DisconnectNetwork();
        InitNetwork();
    }

    Dev::Print(Dev::PRINT_NORMAL, "Networking::Run -> Starting network loop...");
    loopThread = std::thread(networkLoop);
}

void SendData(bool32 verify) {
    ServerPacket send;
...
void DisconnectNetwork(bool32 finalClose) {
    if (session->running)
        session->leave();
    if (loopThread.joinable())
        loopThread.join();
...
void SendServerPacket(ServerPacket &send, bool32 repeat) { session->write(send, repeat); }

int32 GetRoomCode() {
    int32 code = session->room;
    Dev::PrintInt32(Dev::PRINT_NORMAL, "Networking::GetRoomCode", code);
    return code;
}
void SetRoomCode(int32 code) { session->room = code; }

void SetNetworkGameName(int32 *a1, const char *name) { Misc::StrCopy(networkGame, name); }
